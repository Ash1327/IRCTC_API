...
async function addTrain(req, res) {
  ...
  if (trains.length === 0) {
    return res.status(400).json({ message: 'Please provide train details to add.' });
  }
...
// updating seats of a train 
async function updateTrainSeats(req, res) {
  ...
  try {
    ...
    if (updated) {
      ...
      res.status(404).json({ message: 'Train or seats not updated' });
    }
  } catch (err) {
